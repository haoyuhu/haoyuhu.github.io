...
const ResumeView: React.FC<ResumeViewProps> = ({ resume, locale, copy }) => {
  const [viewMode, setViewMode] = useState<'visual' | 'json'>('visual');
  const resumeCopy = copy.resumeView ?? {};
    ...
    return (
      <div className="font-mono text-sm">
        <div className="mb-4 flex justify-end">
          ...
          >
            {getLocalizedText(
              resumeCopy.switchToVisual ?? { 'zh-CN': '切换到可视视图', en: 'Switch to Visual View' },
              locale,
            )}
          </button>
        </div>
    ...
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="mb-8 flex items-center justify-between border-b border-ide-border pb-4">
        <div>
          <h2 className="text-xl font-bold text-ide-text">
            {getLocalizedText(
              resumeCopy.title ?? { 'zh-CN': 'git log --graph --all', en: 'git log --graph --all' },
              locale,
            )}
          </h2>
        ...
        >
          {getLocalizedText(resumeCopy.jsonToggle ?? '{ JSON }', locale)}
        </button>
      </div>
      ...
      <div className="mb-12">
        <h3 className="mb-6 flex items-center gap-2 text-lg font-bold text-ide-text">
          ...
          {getLocalizedText(
            resumeCopy.experienceHeading ?? { 'zh-CN': '经历时间线', en: 'EXPERIENCE_HISTORY' },
            locale,
          )}
        </h3>
      ...
      <div className="mb-12">
        <h3 className="mb-6 flex items-center gap-2 text-lg font-bold text-ide-text">
          ...
          {getLocalizedText(
            resumeCopy.educationHeading ?? { 'zh-CN': '教育背景', en: 'EDUCATION_LOG' },
            locale,
          )}
        </h3>
      ...
      <div>
        <h3 className="mb-6 flex items-center gap-2 text-lg font-bold text-ide-text">
          ...
          {getLocalizedText(
            resumeCopy.skillGroupsHeading ?? { 'zh-CN': '技能分组', en: 'SKILL_GROUPS' },
            locale,
          )}
        </h3>
        <div className="grid gap-4 md:grid-cols-2">
